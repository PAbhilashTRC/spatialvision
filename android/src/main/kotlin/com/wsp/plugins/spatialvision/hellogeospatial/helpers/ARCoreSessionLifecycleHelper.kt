...
class ARCoreSessionLifecycleHelper(
  ...
  private fun tryCreateSession(): Session? {
    ...
    if (!GeoPermissionsHelper.hasGeoPermissions(activity.requireActivity())) {
      GeoPermissionsHelper.requestPermissions(activity.requireActivity())
      return null
    }

    return try {
      // Request installation if necessary.
      when (ArCoreApk.getInstance().requestInstall(activity.requireActivity(), !installRequested)!!) {
      ...
      Session(activity.requireActivity(), features)
    } catch (e: Exception) {
