...
//    }
//    /**
//     * Generate insulator mesh between two points (base and tip)
//     */
//    fun generateInsulatorMesh(base: Vec3, tip: Vec3): Pair<List<Vertex>, List<Face>> {
//        val direction = floatArrayOf(
//            tip.x - base.x,
//            tip.y - base.y,
//            tip.z - base.z
...
//        direction[2] /= len
//
//        // Create a cylinder with multiple segments for insulator shape
//        val segments = 16
//        val radiusBottom = 0.012f
//        val radiusTop = 0.008f
//        val radiusMid = 0.015f
...
//
//        // Generate the cylinder with variable radius (typical insulator shape)
//        for (i in 0..segments) {
//            val t = i.toFloat() / segments  // 0 at base, 1 at tip
//            val y = t * height
//
//            // Calculate radius based on position (insulator profile)
//            val radius = when {
//                t < 0.2f -> radiusBottom + (radiusMid - radiusBottom) * (t / 0.2f)
//                t < 0.8f -> radiusMid
//                else -> radiusMid + (radiusTop - radiusMid) * ((t - 0.8f) / 0.2f)
...
//                val nz = kotlin.math.sin(angle)
//
//                // Calculate perpendicular vectors for the ring
//                val up = floatArrayOf(0f, 1f, 0f)
//                val right = MathUtils.normalize(MathUtils.cross(direction, up))
//                val forward = MathUtils.normalize(MathUtils.cross(right, direction))
//
...
//                )
//
//                // Normal is outward from cylinder
//                val normal = MathUtils.normalize(floatArrayOf(offset[0], offset[1], offset[2]))
//
//                vertices.add(Vertex(vertex, floatArrayOf(normal[0], normal[1], normal[2])))
//                ringVertices.add(vertices.size - 1)
//            }
//
//            // Create faces connecting this ring to the previous ring
//            if (i > 0) {
//                val prevRingStart = (i - 1) * (segments + 1)
//                for (j in 0 until segments) {
//                    val a = prevRingStart + j
//                    val b = prevRingStart + j + 1
//                    val c = prevRingStart + segments + 1 + j
//                    val d = prevRingStart + segments + 1 + j + 1
//
//                    faces.add(Face(intArrayOf(a, b, d)))
//                    faces.add(Face(intArrayOf(a, d, c)))
//                }
//            }
//        }
//
...
//
//    /**
//     * Generate bolt mesh at a point
//     */
//    fun generateBoltMesh(center: Vec3, radius: Float = 0.008f, height: Float = 0.012f): Pair<List<Vertex>, List<Face>> {
//        val segments = 12
//        val vertices = mutableListOf<Vertex>()
//        val faces = mutableListOf<Face>()
//
...
//        val shaftHeight = height * 0.4f
//
//        // Generate bolt head (cylinder with flat top)
//        val bottomCenter = Vec3(center.x, center.y - height/2, center.z)
//        val topCenter = Vec3(center.x, center.y + height/2, center.z)
//
//        // Add cylinder for bolt head
//        val (headVerts, headFaces) = generateCylinderMeshWorld(
//            Vec3(center.x, center.y + shaftHeight/2, center.z),
//            Vec3(center.x, center.y + shaftHeight/2 + headHeight, center.z),
//            headRadius,
...
//        vertices.addAll(headVerts)
//        vertices.addAll(shaftVerts)
//        faces.addAll(headFaces)
//        faces.addAll(shaftFaces)
//
//        return Pair(vertices, faces)
//    }
//
//    /**
//     * Generate insulator mesh with direction (legacy method - keep for compatibility)
//     */
////    fun generateInsulatorMesh(center: Vec3, dir: FloatArray): Pair<List<Vertex>, List<Face>> {
////        // Calculate tip point 0.08f along direction
////        val tip = Vec3(
////            center.x + dir[0] * 0.08f,
////            center.y + dir[1] * 0.08f,
////            center.z + dir[2] * 0.08f
////        )
////        return generateInsulatorMesh(center, tip)
////    }
//
//}
//
