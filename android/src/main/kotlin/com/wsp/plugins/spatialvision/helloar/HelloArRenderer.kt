...
//            val anchorPositions = floatArrayOf(anchor.pose.tx(), anchor.pose.ty(), anchor.pose.tz())
        ...
        for (i in 0 until anchorPositions.size - 1 step 2) {
            if (i + 1 < anchorPositions.size) {
                ...
                cylinder?.draw(
                    render = render,
                    start=Vec3(x = anchorPositions[i][0],
                        y= anchorPositions[i][1],
                        z = anchorPositions[i][2]),
                    end =Vec3(x = anchorPositions[i + 1][0],
                        y = anchorPositions[i + 1][1],
                        z = anchorPositions[i + 1][2]),
                    viewMatrix,
                    projectionMatrix,
                    radius = 0.008f,
                    asset = "measuring"
